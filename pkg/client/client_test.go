...
var _ = Describe("Scalability", func() {
	...
	DescribeTable("multi container", func(terminal bool, ctrCount int) {
		tr = newTestRunner()
		tr.createMultiContainerRuntimeConfigWithProcessArgs(terminal, uint(ctrCount), []string{"/busybox", "ls"}, nil)
		sut = tr.configGivenEnv()
		tr.createAllContainers(sut, terminal)
		tr.startAllcontainers()

		rss := vmRSSGivenPID(sut.PID())
		GinkgoWriter.Println("VmRSS for server is", rss)
		// allow around 100KiB per container
		Expect(rss).To(BeNumerically("<", maxRSSKB+ctrCount*100))
	},
